...
/// Image container for processing operations.
...
impl PhotonImage {
	...
	pub fn encode(&self, format: u8, quality: u8) -> Result<Vec<u8>> {
		encode_image(&self.img, format, quality)
	}
	...
	pub fn resize(&self, width: u32, height: u32, filter: SamplingFilter) -> Self {
		Self { img: Arc::new(self.img.resize_exact(width, height, filter.into())) }
	}
}
