...
struct ShellSessionCore {
	shell:         BrushShell,
	current_abort: Option<cancel::AbortToken>,
}
...
impl Shell {
	...
	pub async fn abort(&self) -> Result<()> {
		...
		{
			at.abort(cancel::AbortReason::Signal);
		}
		Ok(())
	}
}
		...
		reason = ct.wait() => {
			...
			*session.lock().await = None;
			return Ok(ShellRunResult {
				exit_code: None,
				cancelled: matches!(reason, cancel::AbortReason::Signal),
				timed_out: matches!(reason, cancel::AbortReason::Timeout),
			});
		}
	};
	let res =
...
async fn run_shell_command(
	...
	let on_chunk: Option<Arc<dyn Fn(String) + Send + Sync>> = on_chunk.map(Arc::from);
...
async fn read_output(
	...
	loop {
		...
		while it > 0 {
			...
			match str::from_utf8(pending) {
				Ok(text) => {
					emit_chunk(text, on_chunk.as_deref());
					it = 0;
					break;
				},
				Err(err) => {
					...
					match err.error_len() {
						Some(p) => {
							...
							emit_chunk(REPLACEMENT, on_chunk.as_deref());
							// copy p..it to the beginning of the buffer
							buf.copy_within(p..it, 0);
							it -= p;
	...
	for chunk in buf[..it].utf8_chunks() {
		...
		if !valid.is_empty() {
			emit_chunk(valid, on_chunk.as_deref());
		}
...
fn emit_chunk(text: &str, callback: Option<&(dyn Fn(String) + Send + Sync)>) {
