...
impl PtySession {
	...
	///
	/// The `on_chunk` callback receives streamed output as strings.
	/// Returns the result when the command completes, or flags for
	...
	pub fn start(
		...
		let ct = CancelToken::new(options.timeout_ms);

		// Register control channel so write()/kill() work immediately.
		...
		let result = run_pty_sync(run_config, on_chunk, control_rx, ct);
	...
	while exit_code.is_none() || !reader_done {
		...
		for _ in 0..READER_EVENTS_PER_TICK {
			match reader_rx.try_recv() {
				Ok(ReaderEvent::Chunk(chunk)) => emit_chunk(&chunk, on_chunk.as_deref()),
				Ok(ReaderEvent::Done) => {
					reader_done = true;
					break;
				},
